...
public abstract class UDPConnector extends BIOHandler{
	...
	/**
	 * 构造函数：指定address，建立新的socket连接
	 */
	public UDPConnector(InetSocketAddress targetAddress, ISocketCallback callback) {
		super();
	...
	public boolean start() {
		try {
			if(!super.start() || mUDPSocket == null){
				return false;
			}
...
//				mUDPSocket.setSoTimeout(readTimeout);
			//UDP连接，不会阻塞
//			mUDPSocket.connect(mTargetAddress);	
	...
	public boolean send(OutPacket packet) {
		try {
			if (!isStarted()) {
				throw new SocketUnstartedException("UDP socket didn't start yet!");
			}
