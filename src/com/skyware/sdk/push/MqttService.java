...
public class MqttService extends Service
{
	...
	private static int		MQTT_QUALITY_OF_SERVICE   = 0;		//接收离线消息
	
	// The broker should not retain any messages.
	private static boolean	MQTT_RETAINED_PUBLISH     = false;	//TODO 研究一下
	...
	private static final long	KEEP_ALIVE_INTERVAL = 1000 * 60 * 28;	//28min改为10min
	...
	public void onCreate() {
		...
		mPrefs 		= getSharedPreferences(SDKConst.SP_NAME_MQTT, MODE_PRIVATE);
	...
	/**
	 * 原因：http://www.dewen.io/q/4654
	 * http://blog.csdn.net/wulianghuan/article/details/8596467
	 */
	@Override
	...
	private synchronized void connect() {		
		...
		if (SkySDK.getConfig() != null && SkySDK.getConfig().getUserId() != null &&
				!SkySDK.getConfig().getUserId().equals("")) {
			userTopic = TOPIC_HEADER.concat(SkySDK.getConfig().getUserId());
		}
		...
		if (deviceID == null) {
			...
			try {
				...
				if (mConnection != null && userTopic != null) {
					// 再次订阅userId
					mConnection.subscribeToTopic(userTopic);
				}
...
//			showNotification(s);
			...
			//上报NetworkManager
			NetworkManager nm = BizManager.getInstace().getNetworkManager();
			if (nm != null && nm.getNetCallback() != null) {
				nm.getNetCallback().onReceivePushMqtt(topicName, payload, qos, retained);
			}
		}   
